...
import i_releasable from '../util/i_releasable.js';
 ...
 * @implements {Iterable.<!segment_reference>}
 * @export
 */
 export class segment_index {
  /**
   * @param {!Array.<!segment_reference>} references The list of
  ...
  constructor(references) {
    ...
    // For debugging.
    if (true) {
      segment_index.assertCorrectReferences_(references);
    }

    /** @protected {!Array.<!segment_reference>} */
   ...
   * @param {function(!segment_reference)} fn
   */
   ...
   * @return {segment_reference} The SegmentReference, or null if
  ...
  merge(references) {
    ...
    // For debugging.
    if (true) {
      segment_index.assertCorrectReferences_(this.references);
    }
  }
   ...
   * @param {function():Array.<segment_reference>} updateCallback
   * @export
   */
  ...
  getIteratorForTime(time) {
    ...
    return new segment_iterator(this, index, partialSegmentIndex);
  }
  ...
  static forSingleSegment(startTime, duration, uris) {
    const reference = new segment_reference(
        /* startTime= */ startTime,
    ...
    return new segment_index([reference]);
  }
};
...
if (true) {
   ...
   *
   * @param {!Array.<segment_reference>} references
   * @private
   */
  segment_index.assertCorrectReferences_ = (references) => {
    assert(references.every((r2, i) => {
      if (i == 0) {
        return true;
      }
 ...
 *
 * @implements {Iterator.<segment_reference>}
 * @export
 */
 ...
 *
 * @extends segment_index
 * @implements {IReleasable}
 * @implements {Iterable.<!segment_reference>}
 * @export
 */
export class meta_segment_index extends segment_index {
  /** */
  constructor() {
    super([]);

    /** @private {!Array.<!segment_index>} */
    this.indexes_ = [];
  }
   ...
   *
   * @return {!meta_segment_index}
   */
  clone() {
    const clone = new meta_segment_index();
