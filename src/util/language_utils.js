...
class language_utils {
  ...
  static areLocaleCompatible(locale1, locale2) {
    ...
    // do it again.
    locale1 = language_utils.normalize(locale1);
    locale2 = language_utils.normalize(locale2);
  ...
  static areLanguageCompatible(locale1, locale2) {
    ...
    /** @type {!Array.<string>} */
    const locale1Components = language_utils.disassembleLocale_(locale1);
    /** @type {!Array.<string>} */
    const locale2Components = language_utils.disassembleLocale_(locale2);
  ...
  static isParentOf(possibleParent, possibleChild) {
    // Even through they SHOULD already be normalized, let's just be safe and
    // do it again.
    possibleParent = language_utils.normalize(possibleParent);
    possibleChild = language_utils.normalize(possibleChild);

    // Get all components. This should only be language and region
    // since we do not support dialect.
    /** @type {!Array.<string>} */
    const possibleParentComponents =
    language_utils.disassembleLocale_(possibleParent);
    /** @type {!Array.<string>} */
    const possibleChildComponents =
    language_utils.disassembleLocale_(possibleChild);
   ...
   */
    static isSiblingOf(localeA, localeB) {
      // Even through they SHOULD already be normalized, let's just be safe and
      // do it again.
      localeA = language_utils.normalize(localeA);
      localeB = language_utils.normalize(localeB);
  
      // Get all components. This should only be language and region
      // since we do not support dialect.
      /** @type {!Array.<string>} */
      const localeAComponents = language_utils.disassembleLocale_(localeA);
      /** @type {!Array.<string>} */
      const localeBComponents = language_utils.disassembleLocale_(localeB);
  ...
  static normalize(locale) {
    ...
    language = language_utils.isoMap_.get(language) || language;
   ...
   */
   static areSiblings(a, b) {
    const baseA = language_utils.getBase(a);
    const baseB = language_utils.getBase(b);
   ...
   */
   static relatedness(target, candidate) {
    target = language_utils.normalize(target);
    candidate = language_utils.normalize(candidate);
    ...
    if (language_utils.isParentOf(candidate, target)) {
      return 3;
    }

    // Next is a sibling of the target language.
    if (language_utils.isSiblingOf(candidate, target)) {
      return 2;
    }

    // Next is a child of the target language.
    if (language_utils.isParentOf(target, candidate)) {
      return 1;
    }
   ...
   */
   static getBase(lang) {
    ...
    major = language_utils.isoMap_.get(major) || major;
   ...
   */
   static getLocaleForText(stream) {
    ...
    return language_utils.normalize(language);
  }
  ...
  static getLocaleForVariant(variant) {
    ...
    if (variant.language) {
      return language_utils.normalize(variant.language);
    }

    if (variant.audio && variant.audio.language) {
      return language_utils.normalize(variant.audio.language);
    }

    if (variant.video && variant.video.language) {
      return language_utils.normalize(variant.video.language);
    }
  ...
  static findClosestLocale(target, searchSpace) {
    /** @type {string} */
    const safeTarget = language_utils.normalize(target);
    /** @type {!Set.<string>} */
    const safeSearchSpace = new Set();
    for (const option of searchSpace) {
      safeSearchSpace.add(language_utils.normalize(option));
    }
    ...
    for (const option of safeSearchSpace) {
      if (language_utils.isParentOf(option, safeTarget)) {
        return option;
      }
    }
    ...
    for (const option of safeSearchSpace) {
      if (language_utils.isSiblingOf(option, safeTarget)) {
        return option;
      }
    }
    ...
    for (const option of safeSearchSpace) {
      if (language_utils.isParentOf(safeTarget, option)) {
        return option;
      }
    }
 ...
 * @private
 */
 language_utils.isoMap_ = new Map([
