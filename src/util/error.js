...
class error {
  constructor(severity, category, code, ...varArgs) {
     ...
     *
     */
    this.message = 'Error ' + categoryName + '.' + codeName +
      ' (' + this.data.toString() + ')';
  ...
  toString() {
    return 'util.error ' + JSON.stringify(this, null, '  ');
  }
}

/* Enable createStack only in debug mode */
...
error.Category = {
  ...
  /** Errors related to segment manipulation */
  'SEGMENT': 11,
};
...
error.Code = {
  ...
  /**
   * Segment manipulation failed.
   */
  'SEGMENT_MANIPULATION_FAILED': 10005,
};
