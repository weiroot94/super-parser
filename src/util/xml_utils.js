...
import { node_constants } from '../constants/node_consts.js';
...
class xml_utils {
  ...
  static findChildren(elem, name) {
    ...
    for (i; i < childs.length; i++) {
      if (childs[i] instanceof Object && childs[i].nodeName == name) {
        found.push(childs[i]);
      }
    }
    return found;
  ...
  static findChildrenNS(elem, ns, name) {
    ...
    for(i; i < childs.length; i++) {
      if (childs[i] instanceof Object &&
          childs[i].localName == name &&
          childs[i].namespaceURI == ns) {
        found.push(childs[i]);
      }
    }
    return found;
  ...
  static getContents(elem) {
    if (!Array.from(elem.childNodes).every(xml_utils.isText)) {
      return null;
    }
   ...
   */
  static isText(elem) {
    return elem.nodeType == node_constants.text_node ||
      elem.nodeType == node_constants.cdata_section_node;
  }
  ...
  static parseDuration(durationString) {
    ...
    if (!matches) {
      logger.sp_warn(filePath, 'Invalid duration string: %s', durationString);
      return null;
    }
  ...
  static parseXmlString(xmlString, expectedRootElemName) {
    ...
    try {
      ...
      logger.sp_error(filePath, 'XML parsing exception: %s', exception);
      return null;
    }
    ...
    if (!rootElem) {
      logger.sp_error(filePath, 'XML document was empty!');
      return null;
    }
    ...
    if (parserErrorElements.length) {
      logger.sp_error(filePath, 'XML parser error found: %s', parserErrorElements[0]);
      return null;
    }
    ...
    if (xml.documentElement.tagName != expectedRootElemName) {
      logger.sp_error(filePath, 
        `XML tag name does not match expected "${expectedRootElemName}": %s`,
  ...
  static parseXml(data, expectedRootElemName) {
    try {
      return xml_utils.parseXmlString(data, expectedRootElemName);
    } catch (exception) {
      logger.sp_log('parseXmlString threw! %s', exception);
      return null;
    }
  }
}
