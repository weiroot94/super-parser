...
import drm_engine from '../media/drm_engine.js';
import { segment_index, meta_segment_index } from '../media/segment_index.js';
import array_utils from './array_utils.js';
import error from './error.js';
import i_releasable from './i_releasable.js';
import language_utils from './language_utils.js';
import manifest_parser_utils from './manifest_parser_utils.js';
import map_utils from './map_utils.js';
import mime_utils from './mime_utils.js';
 ...
 * 
 * @implements {i_releasable}
 * @final
 */
class period_combiner {
  /** */
  constructor() {
  ...
  /**
   * @param {!Array.<period_combiner.Period>} periods
  ...
  async combinePeriods(periods, isDynamic) {
    ...
    period_combiner.filterOutAudioStreamDuplicates_(periods);
    period_combiner.filterOutVideoStreamDuplicates_(periods);
    period_combiner.filterOutTextStreamDuplicates_(periods);
    period_combiner.filterOutImageStreamDuplicates_(periods);
    ...
    if (!isDynamic && periods.length == 1) {
      ...
      for (const textStreams of textStreamsPerPeriod) {
        textStreams.push(period_combiner.dummyStream_(
            ContentType.TEXT));
      }
      for (const imageStreams of imageStreamsPerPeriod) {
        imageStreams.push(period_combiner.dummyStream_(
            ContentType.IMAGE));
      }

      await period_combiner.combine_(
          this.audioStreams_,
          audioStreamsPerPeriod,
          firstNewPeriodIndex,
          period_combiner.cloneStream_,
          period_combiner.concatenateStreams_);
    ...
    if (!this.videoStreams_.length || !this.audioStreams_.length) {
      ...
      for (const audio of this.audioStreams_) {
        for (const video of this.videoStreams_) {
          const commonDrmInfos = drm_engine.getCommonDrmInfos(
      ...
      for (const a1 of period.audioStreams) {
        ...
        for (const a2 of filteredAudios) {
          if (a1.id != a2.id &&
              ...
              array_utils.hasSameElements(a1.roles, a2.roles) &&
      ...
      for (const t1 of period.textStreams) {
        ...
        for (const t2 of filteredTexts) {
          if (t1.id != t2.id &&
            ...
            array_utils.hasSameElements(t1.roles, t2.roles)) {
            duplicate = true;
          }
        }
      ...
      for (const v1 of period.videoStreams) {
        ...
        for (const v2 of filteredVideos) {
          if (v1.id != v2.id &&
              ...
              v1.label == v2.label &&
              array_utils.hasSameElements(v1.roles, v2.roles) &&
              map_utils.hasSameElements(v1.closedCaptions, v2.closedCaptions) &&
   ...
   * updates.
   *
   * @param {!Array.<!Array.<StreamDB>>} streamDbsPerPeriod
   * @return {!Promise.<!Array.<StreamDB>>}
   */
  static async combineDbStreams(streamDbsPerPeriod) {
    ...
    for (const textStreams of textStreamDbsPerPeriod) {
      textStreams.push(period_combiner.dummyStreamDB_(
          ContentType.TEXT));
    }
    for (const imageStreams of imageStreamDbsPerPeriod) {
      imageStreams.push(period_combiner.dummyStreamDB_(
          ContentType.IMAGE));
    }

    const combinedAudioStreamDbs = await period_combiner.combine_(
        /* outputStreams= */ [],
        audioStreamDbsPerPeriod,
        /* firstNewPeriodIndex= */ 0,
        period_combiner.cloneStreamDB_,
        period_combiner.concatenateStreamDBs_);

    const combinedVideoStreamDbs = await period_combiner.combine_(
        /* outputStreams= */ [],
    ...
    const combinedTextStreamDbs = await period_combiner.combine_(
        /* outputStreams= */ [],
    ...
    const combinedImageStreamDbs = await period_combiner.combine_(
        /* outputStreams= */ [],
  ...
  static async combine_(
    ...
    for (const outputStream of outputStreams) {
      ...
      const ok = await period_combiner.extendExistingOutputStream_(
      ...
      if (!ok) {
        ...
        throw new error(
            ...
            error.Code.PERIOD_FLATTENING_FAILED);
      }
    ...
    for (const unusedStreams of unusedStreamsPerPeriod) {
      for (const stream of unusedStreams) {
        ...
        const outputStream =
            period_combiner.createNewOutputStream_(
      ...
      for (const stream of unusedStreams) {
        ...
        const hasCodec = outputStreams.some((s) => {
          return s.mimeType == stream.mimeType &&
                mime_utils.getCodecBase(s.codecs) ==
        ...
        // an error.
        logger.sp_log(filePath, 'Unused stream in period-flattening!',
  ...
  static async extendExistingOutputStream_(
      outputStream, streamsPerPeriod, firstNewPeriodIndex, concat,
      unusedStreamsPerPeriod) {
    period_combiner.findMatchesInAllPeriods_(streamsPerPeriod,
        outputStream);
    ...
    if (!outputStream.matchedStreams) {
      ...
      logger.sp_error(filePath, 'No matches extending output stream!',
    ...
    if (outputStream.segmentIndex) {
      await period_combiner.extendOutputSegmentIndex_(outputStream,
          firstNewPeriodIndex);
    }

    period_combiner.extendOutputStream_(outputStream,
  ...
  static async extendOutputSegmentIndex_(outputStream, firstNewPeriodIndex) {
    ...
    if (outputStream.segmentIndex instanceof meta_segment_index) {
  ...
  static createNewOutputStream_(
    ...
    if (outputStream.createSegmentIndex) {
      ...
      outputStream.createSegmentIndex = async () => {
        if (!outputStream.segmentIndex) {
          outputStream.segmentIndex = new meta_segment_index();
          await period_combiner.extendOutputSegmentIndex_(
  ...
  static extendOutputStream_(
    ...
    const ContentType = manifest_parser_utils.ContentType;
    const LanguageUtils = language_utils;
  ...
  static cloneStream_(stream) {
    const clone = /** @type {Stream} */(Object.assign({}, stream));
   ...
   * periods.
   *
   * @param {StreamDB} streamDb
   * @return {StreamDB}
   * @private
   */
  static cloneStreamDB_(streamDb) {
    const clone = /** @type {StreamDB} */(Object.assign(
        {}, streamDb));
  ...
  static concatenateStreams_(output, input) {
    ...
    if (input.trickModeVideo) {
      if (!output.trickModeVideo) {
        ...
        output.trickModeVideo = period_combiner.cloneStream_(
        ...
        output.trickModeVideo.createSegmentIndex = () => {
          // Satisfy the compiler about the type.
          assert(
              output.segmentIndex instanceof meta_segment_index,
   ...
   *
   * @param {StreamDB} output
   * @param {StreamDB} input
   * @private
   */
  ...
  static findMatchesInAllPeriods_(streamsPerPeriod, outputStream) {
    ...
    for (const streams of streamsPerPeriod) {
      const match = period_combiner.findBestMatchInPeriod_(
  ...
  static findBestMatchInPeriod_(streams, outputStream) {
    const areCompatible = {
      'audio': period_combiner.areAVStreamsCompatible_,
      'video': period_combiner.areAVStreamsCompatible_,
      'text': period_combiner.areTextStreamsCompatible_,
      'image': period_combiner.areImageStreamsCompatible_,
    }[outputStream.type];

    const isBetterMatch = {
      'audio': period_combiner.isAudioStreamBetterMatch_,
      'video': period_combiner.isVideoStreamBetterMatch_,
      'text': period_combiner.isTextStreamBetterMatch_,
      'image': period_combiner.isImageStreamBetterMatch_,
    }[outputStream.type];
  ...
  static areAVStreamsCompatible_(outputStream, candidate) {
    const getCodecBase = (codecs) => mime_utils.getCodecBase(codecs);
    ...
    if (outputStream.drmInfos) {
      ...
      if (!drm_engine.areDrmCompatible(outputStream.drmInfos,
          candidate.drmInfos)) {
        return false;
      }
    }
  ...
  static areTextStreamsCompatible_(outputStream, candidate) {
    ...
    const languageRelatedness = language_utils.relatedness(
  ...
  static isAudioStreamBetterMatch_(outputStream, best, candidate) {
    const LanguageUtils = language_utils;
    const {BETTER, EQUAL, WORSE} = period_combiner.BetterOrWorse;
    ...
    const channelsBetterOrWorse =
        period_combiner.compareClosestPreferLower(
    ...
    if (outputStream.bandwidth) {
      ...
      const bandwidthBetterOrWorse =
          period_combiner.compareClosestPreferMinimalAbsDiff_(
   ...
   * exist in any period.
   *
   * @param {manifest_parser_utils.ContentType} type
   * @return {StreamDB}
   * @private
   */
   ...
   * @return {period_combiner.BetterOrWorse}
   */
 ...
 *   textStreams: !Array.<Stream>,
 *   imageStreams: !Array.<Stream>
 * }}
 *
 ...
 * @property {string} id
 *   The Period ID.
 * @property {!Array.<Stream>} audioStreams
 *   The audio streams from one Period.
 * @property {!Array.<Stream>} videoStreams
 *   The video streams from one Period.
 * @property {!Array.<Stream>} textStreams
 *   The text streams from one Period.
 * @property {!Array.<Stream>} imageStreams
 *   The image streams from one Period.
 */
period_combiner.Period;

/**
 * @enum {number}
 */
period_combiner.BetterOrWorse = {
  BETTER: 1,
  EQUAL: 0,
  WORSE: -1,
};

export default period_combiner;