class buffer_utils {
  ...
  static equal(arr1, arr2) {
    ...
    if (buffer_utils.unsafeGetArrayBuffer_(arr1) ==
      buffer_utils.unsafeGetArrayBuffer_(arr2) &&
      (arr1.byteOffset || 0) == (arr2.byteOffset || 0)) {
      return true;
    }

    const uint8A = buffer_utils.toUint8(arr1);
    const uint8B = buffer_utils.toUint8(arr2);
   ...
   */
   static toUint8(data, offset = 0, length = Infinity) {
    return buffer_utils.view_(data, offset, length, Uint8Array);
  }
   ...
   */
   static toDataView(buffer, offset = 0, length = Infinity) {
    return buffer_utils.view_(buffer, offset, length, DataView);
  }
   ...
   */
   static view_(data, offset, length, Type) {
    const buffer = buffer_utils.unsafeGetArrayBuffer_(data);
