...
import i_destroyable from '../util/i_destroyable.js';
 ...
 * @implements {i_destroyable}
 * @export
 */
 class text_engine {
  /** @param {TextDisplayer} displayer */
   ...
   * @param {!TextParserPlugin} plugin
   * @export
   */
  static registerParser(mimeType, plugin) {
    text_engine.parserMap_[mimeType] = plugin;
  }
  ...
  static unregisterParser(mimeType) {
    delete text_engine.parserMap_[mimeType];
  }
  ...
  static findParser(mimeType) {
    return text_engine.parserMap_[mimeType];
  }
  ...
  static isTypeSupported(mimeType) {
    if (text_engine.parserMap_[mimeType]) {
      // An actual parser is available.
      return true;
    }
    if (mimeType == mime_utils.CEA608_CLOSED_CAPTION_MIMETYPE ||
        mimeType == mime_utils.CEA708_CLOSED_CAPTION_MIMETYPE ) {
      // Closed captions.
      return true;
    }
    return false;
  ...
  initParser(mimeType, sequenceMode, segmentRelativeVttTiming) {
    ...
    const factory = text_engine.parserMap_[mimeType];
    assert(
  ...
  async appendBuffer(buffer, startTime, endTime) {
    ...
    if (startTime == null || endTime == null) {
      this.parser_.parseInit(buffer_utils.toUint8(buffer));
      return;
    }
    ...
    const allCues = this.parser_.parseMedia(
        buffer_utils.toUint8(buffer), time);
  ...
  * @return {!Array<!ICaptionDecoder.ClosedCaption>}
  */
   ...
   *
   * @param {!Array.<!ICaptionDecoder.ClosedCaption>} closedCaptions
   * @param {?number} startTime relative to the start of the presentation
   * @param {?number} endTime relative to the start of the presentation
   * @param {number} videoTimestampOffset the timestamp offset of the video
  ...
  storeAndAppendClosedCaptions(
    ...
    /** @type {!Map.<string, !Map.<string, !Array.<!cue>>>} */
...
/** @private {!Object.<string, !TextParserPlugin>} */
text_engine.parserMap_ = {};

export default text_engine;