...
class segment_template {
  ...
  static createStreamInfo(
    ...
    const initSegmentReference = segment_template.createInitSegment_(context);
    const info = segment_template.parseSegmentTemplateInfo_(context);

    segment_template.checkSegmentTemplateInfo_(context, info);
    ...
    if (info.indexTemplate) {
      segment_base.checkSegmentIndexSupport(
        context, initSegmentReference);

      return {
        generateSegmentIndex: () => {
          return segment_template.generateSegmentIndexFromIndexTemplate_(
      ...
      return {
        generateSegmentIndex: () => {
          return segment_template.generateSegmentIndexFromDuration_(
      ...
      const references = segment_template.createFromTimeline_(
      ...
      if (segmentIndex) {
        if (shouldFit) {
          ...
          const wrapper = new segment_index(references);
  ...
  *
  * @param {dash_parser.Context} context
  * @return {segment_template.SegmentTemplateInfo}
  * @private
  */
  static parseSegmentTemplateInfo_(context) {
    const segmentInfo =
      mpd_utils.parseSegmentInfo(context, segment_template.fromInheritance_);

    const media = mpd_utils.inheritAttribute(
      context, segment_template.fromInheritance_, 'media');
    const index = mpd_utils.inheritAttribute(
      context, segment_template.fromInheritance_, 'index');
  ...
  *
  * @param {dash_parser.Context} context
  * @param {segment_template.SegmentTemplateInfo} info
  * @private
  */
  static checkSegmentTemplateInfo_(context, info) {
    ...
    if (n == 0) {
      ...
      logger.sp_warn(filePath,
        ...
        context.representation);
      if (info.indexTemplate) {
        logger.sp_log(filePath, 'Using the index URL template by default.');
        info.timeline = null;
        info.segmentDuration = null;
      } else {
        assert(info.timeline, 'There should be a timeline');
        logger.sp_log(filePath, 'Using the SegmentTimeline by default.');
        info.segmentDuration = null;
      }
    }
  ...
  * @param {dash_parser.RequestInitSegmentCallback}
  *     requestInitSegment
  * @param {init_segmentReference} init
  * @param {segment_template.SegmentTemplateInfo} info
  ...
  */
  static generateSegmentIndexFromDuration_(
    ...
    const createReference = (position) => {
      ...
      const ref = new segment_reference(
        segmentStart,
        segmentEnd,
        getUris,
      /* startByte= */ 0,
    ...
    /** @type {segment_index} */
    const segmentIndex = new segment_index(references);
  ...
  */
  static createFromTimeline_(context, info, initSegmentReference) {
    ...
    for (let i = 0; i < info.timeline.length; i++) {
      ...
      references.push(new segment_reference(
        periodStart + start,
        periodStart + end,
        createUris,
      /* startByte= */ 0,
  ...
  */
  static createInitSegment_(context) {
    const initialization = mpd_utils.inheritAttribute(
      context, segment_template.fromInheritance_, 'initialization');

    logger.sp_debug(filePath, `Initialization: ${initialization}`);
    ...
    logger.sp_debug(filePath, `RepresentaionID: ${repId}`);
    logger.sp_debug(filePath, `Bandwidth: ${bandwidth}`);
    logger.sp_debug(filePath, `${baseUris.length} BaseUris(es) detected`);
  
    const getUris = () => {
      assert(initialization, 'Should have returned earler');
      const filledTemplate = mpd_utils.fillUriTemplate(
        initialization, repId, null, bandwidth, null);
      const resolvedUris = manifest_parser_utils.resolveUris(
        baseUris, [filledTemplate]);
      return resolvedUris;
    };
    const qualityInfo = segment_base.createQualityInfo(context);
    return new init_segmentReference(getUris, 0, null, qualityInfo);
  }
}
 ...
 */
segment_template.SegmentTemplateInfo;

export default segment_template;